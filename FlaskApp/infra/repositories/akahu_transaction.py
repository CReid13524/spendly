...
from FlaskApp.domainmodel import AkahuTransaction, Transaction
...
class AkahuTransactionRepository:
    ...
    def get(self, akahu_transaction_id: str) -> AkahuTransaction | None:
        orm = self.session.query(AkahuTransactionORM).filter_by(id=akahu_transaction_id).first()
        if orm is None:
            return None
        user = user_orm_to_domain(orm.user)
        return akahu_transaction_orm_to_domain(orm, user=user)

    def get_connected_transaction_by_id(self, akahu_transaction_id: str) -> Transaction | None:
        orm = self.session.query(AkahuTransactionORM).filter_by(id=akahu_transaction_id).first()
        if orm is None:
            return None
        user = user_orm_to_domain(orm.user)
    ...
    def add_or_update(self, akahu_transaction: AkahuTransaction):
        existing = self.session.query(AkahuTransactionORM).filter_by(id=akahu_transaction.id).first()
        if existing:
